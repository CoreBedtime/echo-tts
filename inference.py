...
def load_model_from_hf(
    ...
    if delete_blockwise_modules:
        state = {
            ...
            if not (
                k.startswith("latent_encoder.")
                or k.startswith("latent_norm")
                or ".wk_latent" in k
                or ".wv_latent" in k
            )
        }
...
def get_text_input_ids_and_mask(
    ...
    encoded_texts = [
        tokenizer_encode(text, normalize=normalize, return_normalized_text=True)
...
@torch.inference_mode()
def ae_reconstruct(
    fish_ae: DAC, pca_state: PCAState, audio: torch.Tensor
) -> torch.Tensor:
    assert audio.ndim == 3 and audio.shape[1] == 1  # (b, 1, length)
    ...
    * 2048,  # (~30 seconds, 1/10 max speaker condition size; max chunk seen in training)
    ...
    for i in range(0, audio.shape[1], audio_chunk_size):
        ...
        if audio_chunk.shape[1] < audio_chunk_size:
            audio_chunk = torch.nn.functional.pad(
                audio_chunk, (0, audio_chunk_size - audio_chunk.shape[1])
            )
    ...
    if pad_to_max and speaker_latent.shape[1] < max_speaker_latent_length:
        speaker_latent = torch.nn.functional.pad(
            speaker_latent,
            (0, 0, 0, max_speaker_latent_length - speaker_latent.shape[1]),
        )
    ...
    if divis_by_patch_size is not None:
        speaker_latent = speaker_latent[
            :, : speaker_latent.shape[1] // divis_by_patch_size * divis_by_patch_size
        ]
        speaker_mask = speaker_mask[
            :, : speaker_mask.shape[1] // divis_by_patch_size * divis_by_patch_size
        ]
...
def find_flattening_point(data, target_value=0.0, window_size=20, std_threshold=0.05):
    ...
    padded_data = torch.cat(
        [
            data,
            torch.zeros(
                window_size, *data.shape[1:], device=data.device, dtype=data.dtype
            ),
        ]
    )
...
def crop_audio_to_flattening_point(
    audio: torch.Tensor, latent: torch.Tensor
) -> torch.Tensor:
...
SampleFn = Callable[
    [EchoDiT, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, int], torch.Tensor
]
...
def sample_pipeline(
    ...
    MAX_SPEAKER_LATENT_LENGTH = (
        6400  # max seen during training, though maybe can go higher?
    )
    MAX_TEXT_LENGTH = 768
    ...
    if speaker_audio is None:
        speaker_latent = torch.zeros(
            (1, pad_to_max_speaker_latent_length or 4, 80), device=device, dtype=dtype
        )
        speaker_mask = torch.zeros(
            (1, pad_to_max_speaker_latent_length or 4), device=device, dtype=torch.bool
        )
    else:
        speaker_latent, speaker_mask = get_speaker_latent_and_mask(
            ...
            max_speaker_latent_length=pad_to_max_speaker_latent_length
            or MAX_SPEAKER_LATENT_LENGTH,
            pad_to_max=(pad_to_max_speaker_latent_length is not None),
        )

    latent_out = sample_fn(
        model, speaker_latent, speaker_mask, text_input_ids, text_mask, rng_seed
    )
...
def sample_euler_cfg_independent_guidances(
    ...
    full_speaker_mask = torch.cat(
        [speaker_mask, speaker_mask, speaker_mask_uncond], dim=0
    )

    x_t = torch.randn(
        (batch_size, sequence_length, 80),
        device=device,
        dtype=torch.float32,
        generator=rng,
    )
    ...
    for i in range(num_steps):
        ...
        if has_cfg:
            v_cond, v_uncond_text, v_uncond_speaker = (
                model(
            ...
            )  # can also use a single, joint unconditional for fewer NFE
        else:
            v_pred = model(
                x=x_t.to(dtype),
        ...
        if (
            ...
            _multiply_kv_cache(
                kv_speaker_cond, 1.0 / speaker_kv_scale, speaker_kv_max_layers
            )
            kv_speaker_full = _concat_kv_caches(
                kv_speaker_cond, kv_speaker_cond, kv_speaker_cond
            )
...
if __name__ == "__main__":
    ...
    text_input_ids, text_mask = get_text_input_ids_and_mask(
        [text], max_length=None, device="cuda"
    )
